...
const Gyear_layout			=	"2006"
const Gdate_layout			=	"2006-01-02"
const Gtime_layout			=	"2006-01-02 15:04:05"
...
func NowDate() string {
	return time.Now().Format(Gtime_layout)
}

func NowDateFormat(format string) string {
...
/**
  @param: first httpRequest, second uploaded arr key, third is to save uploaded file full dir path
 *@return: first is uploaded filename, second is save full file path, third is err description.
 */
func SaveUploadFileWithPath(r *http.Request, rFileKey string, dirPath string, isFilenameFormat bool) (string, string, error) {
	var fullFilePath string
	file, header, errFormFile := r.FormFile(rFileKey)
	fileName := header.Filename
	if isFilenameFormat {
		fileName = SetStringDatePrefix(fileName)
	}
	fileBaseName, _ := GetFileBaseExtName(fileName)
	if errFormFile != nil {
		return fileBaseName, fullFilePath, errFormFile
	}
	fullFilePath = dirPath + fileName
	dst, errCreate := os.Create(fullFilePath)
	defer dst.Close()
	if nil != errCreate {
		return fileBaseName, fullFilePath, errCreate
	}

	_, errCopy := io.Copy(dst, file)

	if nil != errCopy {
		return fileBaseName, fullFilePath, errCopy
	}
	return fileBaseName, fullFilePath, nil
}

func GetFileBaseExtName(pathName string) (string, string) {
	baseName := path.Base(pathName)
	suffixName := path.Ext(baseName)
	return baseName[0:len(baseName) - len(suffixName)], suffixName
}

func SetStringDatePrefix(postfix string) string {
	nowDate := NowDateFormat("2006-01-02")

	return fmt.Sprintf("%s_%s", nowDate, postfix)
}